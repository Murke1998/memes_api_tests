...
@pytest.fixture()
def new_meme(create_meme_endpoint,
             delete_meme_endpoint, get_meme_endpoint,
             token):
    body = {'text': 'Qa test',
            'url': 'My home my home',
            'tags': ['Me', 'wife'],
            'info': {'grand': ['grandM', 'grandF']}}
    create_meme_endpoint.create_meme(body, token)
    create_meme_endpoint.check_status_code(200)

    meme_id = create_meme_endpoint.json_response["id"]

    yield meme_id, body

    get_meme_endpoint.get_meme(meme_id, token)

    if get_meme_endpoint.response.status_code == 200:
